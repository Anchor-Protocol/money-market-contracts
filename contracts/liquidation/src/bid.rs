...
use crate::state::{read_bid, read_bids, read_config, remove_bid, store_bid, Bid, Config};
...
pub fn submit_bid<S: Storage, A: Api, Q: Querier>(
    ...
    if read_bid(&deps.storage, &collateral_token_raw, &bidder_raw).is_ok() {
    ...
    let amount: Uint256 = Uint256::from(
        env.message
            ...
            .map(|c| c.amount)
            .ok_or(StdError::generic_err("Must provide stable_denom asset"))
            .unwrap(),
    );
...
pub fn retract_bid<S: Storage, A: Api, Q: Querier>(
    ...
    let collateral_token_raw = deps.api.canonical_address(&collateral_token)?;
    let bidder_raw = deps.api.canonical_address(&env.message.sender)?;
    let bid: Bid = read_bid(&deps.storage, &collateral_token_raw, &bidder_raw)?;
...
pub fn execute_bid<S: Storage, A: Api, Q: Querier>(
    ...
    let collateral_value = amount * Decimal256::from(price.rate);
    let required_stable = collateral_value - collateral_value * bid.premium_rate;
    ...
    if bid.amount == required_stable {
        remove_bid(&mut deps.storage, &collateral_token_raw, &bidder_raw);
    } else {
        store_bid(
            &mut deps.storage,
            &collateral_token_raw,
...
pub fn query_bids<S: Storage, A: Api, Q: Querier>(
    deps: &Extern<S, A, Q>,
    collateral_token: HumanAddr,
    ...
    let bids: Vec<BidResponse> = read_bids(
        &deps,
        &deps.api.canonical_address(&collateral_token)?,
        start_after,
        limit,
