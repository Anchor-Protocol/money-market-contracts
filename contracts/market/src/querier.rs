...
use moneymarket::vterra::{QueryMsg as VeATerraQueryMsg, StateResponse};
...
pub fn query_vterra_state(deps: Deps, vterra_contract: Addr) -> StdResult<StateResponse> {
    deps.querier.query(&QueryRequest::Wasm(WasmQuery::Smart {
...
pub fn query_premium_rate(deps: Deps, vterra_contract: Addr) -> StdResult<Decimal256> {
    query_vterra_state(deps, vterra_contract).map(|state| state.premium_rate)
}
